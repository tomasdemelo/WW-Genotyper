...
1. The first step is obtaining reference sequences of known genotypes from NCBI Virus or similar repositories, and the sequence names must ONLY be the genotype.
...
3. Once that is done, a consensus sequence of each genotype can be generated using the FASTA-seq_condenser.sh, which will act as the backbone for the remainder of the pipeline.

4. After this, ensure the generated consensus sequence is in the same path as the raw reads and raw read pipeline (Raw-read_pipeline.sh), after which the raw read pipeline can be run.
