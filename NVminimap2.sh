  ...
  for i in $(seq -w 1 24); do
    ...
    if [[ "$file" == *"$barcode"* ]]; then
      ...
      if ! minimap2 -ax map-ont EV-cross-contam.fasta "$tmp_filtered" | \
         ...
         tee -a "$fqout" | \
         minimap2 -ax map-ont v3-all_NV_types_combined.fasta - | \
      ...
      if ! minimap2 -ax map-ont v3-all_NV_types_combined.fasta "$fqout" | \
         samtools view -F 4 -b | \
      ...
      if ! samtools mpileup \
            -aa -A -d 0 -Q 0 \
            -f v3-all_NV_types_combined.fasta \
            "$bam_sorted" | \
         ivar variants \
            -p "iVar/${barcode}_variants" \
            -q 20 -t 0.05 -m 400 \ #m is min depth
            -r v3-all_NV_types_combined.fasta \
